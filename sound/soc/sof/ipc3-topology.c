...
static int sof_ipc3_control_load_bytes(struct snd_sof_dev *sdev, struct snd_sof_control *scontrol)
{
	...
	if (scontrol->priv_size > 0) {
		...
		if (SOF_ABI_VERSION_INCOMPATIBLE(SOF_ABI_VERSION, cdata->data->abi)) {
			dev_err(sdev->dev, "%s: Incompatible ABI version 0x%08x.\n",
				__func__, cdata->data->abi);
			ret = -EINVAL;
			goto err;
		}
...
static int sof_ipc3_dai_config(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget,
			       unsigned int flags, struct snd_sof_dai_config_data)
{
	 ...
	 * SOF_DAI_CONFIG_FLAGS_HW_FREE and contains no quirks
	 *
	 * The DAI_CONFIG IPC is sent to the DSP, only after the widget is set up in the DSP.
	 * But since the BE DAI hw_params precedes the FE DAI hw_params, the quirks
