...
/**
 * cdns_pcie_find_capability - Find a PCI capability in config space
 * @addr: Base address of PCI config space (iomem)
...
u8 cdns_pcie_find_capability(void __iomem *addr, u8 cap)
{
	...
	u16 status;
	u8 id;
	int ttl = 48;  /* 48 capabilities max */

	status = readw(addr + PCI_STATUS);
	...
	while (ttl--) {
		if (pos < 0x40)
			break;
		pos &= ~3;
		id = readb(addr + pos + PCI_CAP_LIST_ID);
		if (id == 0xff)
			break;
		if (id == cap)
			return pos;
		pos = readb(addr + pos + PCI_CAP_LIST_NEXT);
	}
...
/**
 * cdns_pcie_find_ext_capability - Find a PCI Express extended capability
 * @addr: Base address of PCI config space (iomem)
...
{
	u32 header;
	int ttl = 480;  /* 3840 bytes / 8 bytes per cap */
