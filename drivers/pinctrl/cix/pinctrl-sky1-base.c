				...
				unsigned *num_maps_out)
{
	...
	case PINCTRL_ACPI_PIN_FUNCTION:
		...
		new_map->data.mux.function = fun_desc->func.name;
		break;
				    ...
				    u32 index)
{
	...
	npins = size / SKY1_PIN_SIZE;
	grp->grp.npins = npins;
	grp->data = devm_kcalloc(spctl->dev,
				 npins, sizeof(struct sky1_pin),
				 GFP_KERNEL);
	pins = devm_kcalloc(spctl->dev,
				 npins, sizeof(unsigned int),
				 GFP_KERNEL);
	if (!pins || !grp->data)
		return -ENOMEM;
	...
	for (i = 0; i < npins; i++) {
		pin = &((struct sky1_pin *)(grp->data))[i];
		sky1_pinctrl_parse_pin(spctl, &pins[i],
				           pin, &list, np);
	}
	grp->grp.pins = pins;
			       ...
			       struct group_desc **group)
{
	...
	npins = acpi_grp->num_pins;
	grp->grp.name = devm_kstrdup(spctl->dev, acpi_grp->name, GFP_KERNEL);
	grp->grp.npins = npins;
	grp->data = devm_kcalloc(spctl->dev, npins,
				 sizeof(struct sky1_pin),
				 GFP_KERNEL);
	pins = devm_kcalloc(spctl->dev, npins,
				 sizeof(unsigned int),
				 GFP_KERNEL);
	if (!pins || !grp->data)
		return -ENOMEM;
				  ...
				  struct sky1_pinctrl *spctl)
{
	...
	function->func.ngroups = num_groups;
	function->func.name = dev_name(&pdev->dev);
	...
	list_for_each_entry_safe(acpi_grp, temp, &pinctrl_group_list, list) {
		...
		if (!ret)
			group_names[index++] = group->grp.name;
