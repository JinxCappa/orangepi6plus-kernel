...
{
	struct armcb_spi *xspi = spi_controller_get_devdata(spi->controller);
	u32 ctrl_reg;
	...
	if (is_high) {
		...
		ctrl_reg &= ~ARMCB_SPI_CR_SSCTRL;
		if (!(xspi->is_decoded_cs))
			ctrl_reg |= ((~(ARMCB_SPI_SS0 << spi_get_chipselect(spi, 0)))
					 << ARMCB_SPI_SS_SHIFT) &
					ARMCB_SPI_CR_SSCTRL;
		else
			ctrl_reg |= (spi_get_chipselect(spi, 0) << ARMCB_SPI_SS_SHIFT) &
					ARMCB_SPI_CR_SSCTRL;
	}
	armcb_spi_write(xspi, ARMCB_SPI_CR, ctrl_reg);
}
...
static irqreturn_t armcb_spi_irq(int irq, void *dev_id)
{
	...
	struct armcb_spi *xspi = spi_controller_get_devdata(ctlr);
...
static int armcb_prepare_message(struct spi_controller *ctlr,
 ...
 */
static int armcb_transfer_one(struct spi_controller *ctlr, struct spi_device *spi,
				  struct spi_transfer *transfer)
{
	struct armcb_spi *xspi = spi_controller_get_devdata(ctlr);
 ...
 * Return:	0 always
 */
static int armcb_prepare_transfer_hardware(struct spi_controller *ctlr)
{
	struct armcb_spi *xspi = spi_controller_get_devdata(ctlr);
 ...
 * Return:	0 always
 */
static int armcb_unprepare_transfer_hardware(struct spi_controller *ctlr)
{
	struct armcb_spi *xspi = spi_controller_get_devdata(ctlr);
...
static int armcb_spi_setup(struct spi_device *spi)
{
	...
	/* GPIO chip selects are now managed by the SPI core */
...
static int armcb_spi_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		ctlr->num_chipselect = ARMCB_SPI_DEFAULT_NUM_CS;
	else
	...
	ctlr->prepare_transfer_hardware = armcb_prepare_transfer_hardware;
	ctlr->prepare_message = armcb_prepare_message;
	ctlr->transfer_one = armcb_transfer_one;
	ctlr->unprepare_transfer_hardware = armcb_unprepare_transfer_hardware;
	ctlr->set_cs = armcb_spi_chipselect;
	ctlr->setup = armcb_spi_setup;
	ctlr->cleanup = armcb_spi_cleanup;
	ctlr->auto_runtime_pm = true;
	ctlr->mode_bits = SPI_CPOL | SPI_CPHA;

	/* Set to default valid value */
	ctlr->max_speed_hz = clk_get_rate(xspi->ref_clk) / 4;
	...
	if (ret) {
		LOG(LOG_ERR, "spi_register_controller failed");
		goto clk_dis_all;
	}
 ...
 * Return:	0 on success and error value on error
 */
static void armcb_spi_remove(struct platform_device *pdev)
{
#ifndef QEMU_ON_VEXPRESS
