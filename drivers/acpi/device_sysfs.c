			       ...
			       int size)
{
	...
	/* Query ACPI device info to check for _UID */
	...
	if (acpi_valid & ACPI_VALID_UID) {
		/* Use UID-based modalias for devices with _UID */
		...
		/* Use CID-based modalias for devices without _UID */
		int start_len = len;
		...
		/* Trim trailing colon only if CIDs were actually added */
		if (len > start_len)
			len--;
	}
