...
static struct cdns_xspi_driver_data cix_driver_data = {
	.mrvl_hw_overlay = false,
	.magic_num = 0x6523,
	.has_platform_clocks = true,
	.has_runtime_pm = true,
};
...
static int cdns_xspi_controller_init(struct cdns_xspi_dev *cdns_xspi)
{
	...
	u32 ctrl_features;
	u16 hw_magic_num;
	u32 expected_magic_num;

	ctrl_ver = readl(cdns_xspi->iobase + CDNS_XSPI_CTRL_VERSION_REG);
	hw_magic_num = FIELD_GET(CDNS_XSPI_MAGIC_NUM, ctrl_ver);

	/* Use driver_data magic_num if set, otherwise use default */
	expected_magic_num = cdns_xspi->driver_data->magic_num ?
			     cdns_xspi->driver_data->magic_num :
			     CDNS_XSPI_MAGIC_NUM_VALUE;

	if (hw_magic_num != expected_magic_num) {
		dev_err(cdns_xspi->dev,
			"Incorrect XSPI magic number: %x, expected: %x\n",
			hw_magic_num, expected_magic_num);
		return -EIO;
	}
...
static void cix_xspi_sdma_handle(struct cdns_xspi_dev *cdns_xspi)
{
				    ...
				    const struct spi_mem_op *op)
{
	...
	if (cdns_xspi->driver_data->has_runtime_pm) {
...
static int cdns_xspi_probe(struct platform_device *pdev)
{
	...
	if (cdns_xspi->driver_data->mrvl_hw_overlay) {
		...
		/* Use Cix SDMA handler for Cix hardware */
		if (cdns_xspi->driver_data == &cix_driver_data)
			cdns_xspi->sdma_handler = &cix_xspi_sdma_handle;
		else
	...
	/* Optional pinctrl - device core handles automatically */
	...
	/* Get and enable clocks if platform requires them */
	if (cdns_xspi->driver_data->has_platform_clocks) {
	...
	/* Get and assert/deassert reset if available */
	...
	/* Enable runtime PM if driver requires it */
	if (cdns_xspi->driver_data->has_runtime_pm) {
...
static void cdns_xspi_master_unprepare_clks(struct cdns_xspi_dev *cdns_xspi)
{
	if (!cdns_xspi->driver_data->has_platform_clocks)
		return;
...
static int __maybe_unused cdns_xspi_runtime_resume(struct device *dev)
{
	...
	return cdns_xspi_master_prepare_clks(cdns_xspi);
}
...
static const struct of_device_id cdns_xspi_of_match[] = {
	...
	{
		.compatible = "cix,xspi-nor",
		.data = &cix_driver_data,
	},
	{ /* end of table */}
};
...
static const struct acpi_device_id cdns_xspi_acpi_match[] = {
	{ "CIXH2002", (kernel_ulong_t)&cix_driver_data },
	{ },
};
