...
unsigned long dst_get_phy_addr(unsigned long addr)
{
	...
	if (is_ttbr0_addr(addr)) {
		/* User space address - use get_user_pages_fast */
		DST_DBG("user addr: 0x%lx\n", addr);
		ret = get_user_pages_fast(addr, 1, 0, &page);
		if (ret <= 0 || !page) {
			DST_DBG("get_user_pages_fast failed: %d\n", ret);
			return 0;
		}
		val = page_to_phys(page) | (addr & ~PAGE_MASK);
		put_page(page);
	} else if (is_ttbr1_addr(addr)) {
		/* Kernel space address - use virt_to_phys directly */
		DST_DBG("kernel addr: 0x%lx\n", addr);
		if (virt_addr_valid((void *)addr))
			val = virt_to_phys((void *)addr);
		else
			DST_DBG("invalid kernel virt addr\n");
	}

	DST_DBG("phys addr: 0x%lx\n", val);
	return val;
}
