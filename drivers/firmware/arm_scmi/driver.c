					 ...
					 int prot_id, const char *name)
{
	...
	sdev = scmi_device_create(of_fwnode_handle(np), info->dev, prot_id, name);
	if (name && !sdev)
		dev_err(info->dev,
			   ...
			   int prot_id, bool tx)
{
	...
	if (!info->desc->ops->chan_available(of_fwnode_handle(of_node), idx)) {
	...
	tdev = scmi_device_create(of_fwnode_handle(of_node), info->dev, prot_id, name);
	if (!tdev) {
		dev_err(info->dev,
...
extern struct scmi_desc scmi_mailbox_desc;
#endif
