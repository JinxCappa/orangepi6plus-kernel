...
/* ICM42670 MREG (Memory Register) access registers */
#define INV_ICM42670_REG_BLK_SEL_W		0x79
#define INV_ICM42670_REG_MADDR_W		0x7A
#define INV_ICM42670_REG_M_W			0x7B
#define INV_ICM42670_REG_BLK_SEL_R		0x7C
#define INV_ICM42670_REG_MADDR_R		0x7D
#define INV_ICM42670_REG_M_R			0x7E
...
/**
 * inv_icm42670_mreg_read - Read from ICM42670 MREG register
 * @st: driver internal state
 * @reg: MREG register address to read
 * @val: pointer to store read value
 *
 * Returns 0 on success, negative error code otherwise.
 */
static int inv_icm42670_mreg_read(struct inv_icm42600_state *st,
				  unsigned int reg, unsigned int *val)
{
	int ret;

	/* BLK_SEL_R must be set to 0 */
	ret = regmap_write(st->map, INV_ICM42670_REG_BLK_SEL_R, 0);
	if (ret)
		return ret;

	/* MADDR_R must be set to the address of the MREG register being accessed */
	ret = regmap_write(st->map, INV_ICM42670_REG_MADDR_R, reg);
	if (ret)
		return ret;
	...
	/* Read from M_R */
	ret = regmap_read(st->map, INV_ICM42670_REG_M_R, val);
	if (ret)
		return ret;
...
/**
 * inv_icm42670_mreg_write - Write to ICM42670 MREG register
 * @st: driver internal state
 * @reg: MREG register address to write
 * @val: value to write
 *
 * Returns 0 on success, negative error code otherwise.
 */
static int inv_icm42670_mreg_write(struct inv_icm42600_state *st,
				   unsigned int reg, unsigned int val)
{
	int ret;

	/* BLK_SEL_W must be set to 0 */
	ret = regmap_write(st->map, INV_ICM42670_REG_BLK_SEL_W, 0);
	if (ret)
		return ret;

	/* MADDR_W must be set to the address of the MREG register being accessed */
	ret = regmap_write(st->map, INV_ICM42670_REG_MADDR_W, reg);
	if (ret)
		return ret;

	/* M_W must be set to the desired value */
	ret = regmap_write(st->map, INV_ICM42670_REG_M_W, val);
	if (ret)
		return ret;
...
/**
 * inv_icm42670_mreg_update_bits - Update bits in ICM42670 MREG register
 * @st: driver internal state
 * @reg: MREG register address
 * @mask: bitmask to update
 * @val: value to write (already shifted to correct position)
 *
 * Returns 0 on success, negative error code otherwise.
 */
static int inv_icm42670_mreg_update_bits(struct inv_icm42600_state *st,
					 ...
					 unsigned int val)
{
	...
	/* Read original value */
	ret = inv_icm42670_mreg_read(st, reg, &orig_val);
	if (ret)
		return ret;

	/* Apply mask and new value */
	val = (orig_val & ~mask) | (val & mask);

	/* Write updated value */
	return inv_icm42670_mreg_write(st, reg, val);
}
				    ...
				    unsigned int fifo_en)
{
	...
	/*
	 * ICM42670 does not have TEMP_EN bit in FIFO_CONFIG1.
	 * For other chips, include it in the mask.
	 */
	if (st->chip != INV_CHIP_ICM42670)
		mask |= INV_ICM42600_FIFO_CONFIG1_TEMP_EN;
	...
	if (fifo_en & INV_ICM42600_SENSOR_TEMP && st->chip != INV_CHIP_ICM42670)
		val |= INV_ICM42600_FIFO_CONFIG1_TEMP_EN;

	/*
	 * ICM42670 requires MREG protocol to access FIFO_CONFIG1.
	 * Other chips use standard regmap access.
	 */
	if (st->chip == INV_CHIP_ICM42670)
		ret = inv_icm42670_mreg_update_bits(st, INV_ICM42600_REG_FIFO_CONFIG1,
						    mask, val);
	else
...
int inv_icm42600_buffer_init(struct inv_icm42600_state *st)
{
	 ...
	 */
	if (st->chip == INV_CHIP_ICM42670)
		return inv_icm42670_mreg_update_bits(st, INV_ICM42600_REG_FIFO_CONFIG1,
