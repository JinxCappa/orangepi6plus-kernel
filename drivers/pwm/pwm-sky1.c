...
static int pwm_sky1_probe(struct platform_device *pdev)
{
	...
	chip = devm_pwmchip_alloc(&pdev->dev, 1, sizeof(*sky1));
	if (IS_ERR(chip))
		return PTR_ERR(chip);
	...
	if (screen_info.lfb_linelength && (pwmcr & (PWM_MODE|ENABLE|ENPWM)))
		return devm_pwmchip_add(&pdev->dev, chip); /* already init in uefi */
...
{
	/* With devm_pwmchip_add, cleanup is automatic */
}
